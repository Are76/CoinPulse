...
| `GET /api/transactions` | DTO-read | `app/api/transactions/route.ts` | `src/services/transactions/transaction-service.ts`; wallet lookup in `src/services/api/wallets.ts`; request/error helpers in `src/services/api/validation.ts` | `src/lib/api/transactions-client.ts` | `src/lib/query/use-transactions-query.ts` | `tests/api/transactions-route-contract.test.ts` (`returns 200 with stable { data: TransactionsPageDto } envelope`; `returns 200 with empty transactions and stable pageInfo`; `normalises walletAddress to lowercase`; no-raw-log and chain-aware assetId cases) | `tests/api/transactions-route-contract.test.ts` (walletAddress missing/invalid/too-short; chainId missing/zero/negative/non-numeric; limit exceeds max/fractional/zero; fromDate/toDate invalid) | Not applicable — wallet-not-tracked returns a stable empty page, not 404 | Not applicable | `tests/api/transactions-route-contract.test.ts` (`returns 500 INTERNAL_ERROR with safe envelope and no internal details when service throws`) | complete |
| `POST /api/prices/ingest` | internal operator ingestion route | `app/api/prices/ingest/route.ts` | `src/services/pricing/price-ingestion.ts`; request/error helpers in `src/services/api/validation.ts` | Not applicable — internal ingestion route, no frontend client | Not applicable — no frontend query hook | `tests/api/prices-ingest-route-contract.test.ts` (`returns 200 with schemaVersion v1 on success`; `returns 200 with failedCount when fetch fails for an asset`) | `tests/api/prices-ingest-route-contract.test.ts` (missing required fields; invalid token address; empty assets array; invalid blockNumber; invalid observedAt; tokenDecimals out of range; malformed JSON body) | Not applicable | Not applicable | `tests/api/prices-ingest-route-contract.test.ts` (`response does not expose stack traces or internal error details`) | complete |

## Current V1 coverage gaps

No open route-level contract-test gaps are identified for the current implemented V1 route set. PR #108 closed the previously listed debug health, manual sync, and rebuild validation/not-found/internal-error route-level gaps. PRs #168–#177 added `GET /api/transactions` with full contract coverage.
