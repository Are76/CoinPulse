...
This documentation-only index addresses the G6 blocker in `docs/data-fetching-template-readiness-audit.md`: reviewers need one place to verify current CoinPulse DTO/API route contract-test coverage without re-auditing the test tree manually.
...
| `GET /api/debug/health` | support/status route | `app/api/debug/health/route.ts` | `src/services/debug/health.ts`; request/error helpers in `src/services/api/validation.ts` | `src/lib/api/debug-client.ts` | `src/lib/query/use-debug-health-query.ts` | `tests/api/debug-health-route.test.ts` (`returns backend readiness without secrets`; `returns 503 when a dependency is unavailable`) | Not applicable | Not applicable | Not applicable | Missing exact route-level test for the catch path that returns the safe internal-error envelope when `getHealthReport` throws | mostly complete |
...
| `POST /api/sync/manual` | operator mutation | `app/api/sync/manual/route.ts` | `src/services/sync/index.ts`; wallet lookup in `src/services/api/wallets.ts`; operation conflicts in `src/services/operations/operation-lock.ts`; request/error helpers in `src/services/api/validation.ts` | `src/lib/api/debug-client.ts` | `src/lib/query/use-manual-sync-mutation.ts` | `tests/api/backend-operation-routes.test.ts` (`validates input, resolves the wallet, and delegates to the sync service`) | `tests/api/backend-operation-routes.test.ts` (`returns a structured validation error for invalid sync input`) | Missing exact route-level test for the implemented `WALLET_NOT_FOUND` branch when the requested tracked wallet is absent | `tests/api/backend-operation-routes.test.ts` (`returns a structured 409 conflict when manual sync is blocked by an active rebuild`) | Missing exact route-level test for the catch-all safe internal-error envelope when the sync service or wallet lookup throws an unexpected non-conflict error | mostly complete |
| `POST /api/rebuild` | operator mutation | `app/api/rebuild/route.ts` | `src/services/rebuild/index.ts`; wallet lookup in `src/services/api/wallets.ts`; operation conflicts in `src/services/operations/operation-lock.ts`; request/error helpers in `src/services/api/validation.ts` | `src/lib/api/debug-client.ts` | `src/lib/query/use-rebuild-mutation.ts` | `tests/api/backend-operation-routes.test.ts` (`delegates rebuild and materialization to the backend services`) | Missing exact route-level test for the implemented validation-error branch on invalid rebuild input | Missing exact route-level test for the implemented `WALLET_NOT_FOUND` branch when the requested tracked wallet is absent | `tests/api/backend-operation-routes.test.ts` (`returns a structured 409 conflict when rebuild is blocked by an active sync`) | Missing exact route-level test for the catch-all safe internal-error envelope when rebuild or wallet lookup throws an unexpected non-conflict error | partial |

## Exact missing V1 coverage items

Only the following missing items were identified for the current required route set:

1. `GET /api/debug/health`: add a route-level internal-error test proving the catch path returns the safe error envelope when `getHealthReport` throws.
2. `POST /api/sync/manual`: add a route-level not-found test proving the implemented `WALLET_NOT_FOUND` branch when `resolveTrackedWalletByAddress` returns `null`.
3. `POST /api/sync/manual`: add a route-level internal-error test proving unexpected non-conflict errors return the safe internal-error envelope.
4. `POST /api/rebuild`: add a route-level validation-error test for invalid rebuild input.
5. `POST /api/rebuild`: add a route-level not-found test proving the implemented `WALLET_NOT_FOUND` branch when `resolveTrackedWalletByAddress` returns `null`.
6. `POST /api/rebuild`: add a route-level internal-error test proving unexpected non-conflict errors return the safe internal-error envelope.
...
G6 moves from **mostly met** to **met for coverage visibility** because this index now gives reviewers a single V1 route-by-route view of current route contract coverage and exact missing tests.

G6 should remain **mostly met for complete V1 route coverage** until the six missing tests above are added. The remaining gaps are limited to route-level test coverage for already implemented error branches; no source, schema, DTO, API, pricing, PnL, accounting, frontend UI, RPC, chain-support, template-folder, separate-repo, or extraction work is implied by this document.

Recommended next blocker to address: add the documentation-only hook/mutation test checklist for G7. That is the next smallest safe template-readiness slice because it is documentation-only, does not require route/API changes, and complements this route-contract coverage index.
