...
- **Status vocabulary note (2026-07-24):** the dashboard DTO's `DashboardMetadataProvenanceStatus` type currently contains `verified | observed | conflicting | stale | unknown`, but `computeTokenMetadataStatus` only ever emits `observed | conflicting | stale | unknown` — `verified` is declared in the type but is not reachable from any code path. The broader trust/source policy vocabulary (`unknown, observed, seeded, manual, verified, stale, conflict, rejected`) is a policy model, not a literal one-to-one DTO status enum: note in particular that the DTO uses `conflicting` where the policy uses `conflict`, and the DTO has no `seeded` or `rejected` status at all. `verified`, policy-level `conflict`, `seeded`, `manual`, and `rejected` must not be treated as implemented or reachable merely because they appear in planning docs or in the DTO's type union. Aligning the DTO vocabulary to the policy vocabulary requires a separate, explicit bounded contract decision — not an incidental rename.
