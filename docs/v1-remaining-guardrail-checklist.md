...
- [ ] Refresh stale planning docs or add status notes to them so reviewers do not mistake completed TanStack Query/pricing-status work for remaining work. (Partially done: status notes added to `docs/pnl-accounting-guardrails.md` §9 item 3 and `docs/frontend-query-standardization-audit.md` banner. Still needed: `docs/reusable-backend-template-plan.md` still lists QueryClient/useQuery migration and `GET /api/prices/status` as future steps — status notes there are required before this item can be checked off.)
