...
## D-033: Canonical Identity of a Native pHEX Ended-Stake Observation

**Status:** Accepted (2026-07-24)

**Evidence:** [E2] Migration `20260724000001_add_canonical_identity_unique_to_raw_ended_hex_stake_observation` (adds `@@unique([chainId, walletAddress, stakeId])` on `RawEndedHexStakeObservation`). [E3] `prisma/schema.prisma`, `src/services/hexmining/ended-stake-observation-store.ts`, `src/services/hexmining/ended-stake-discovery.ts`, `app/api/hexmining/ended-stakes/discover/route.ts`. [E1] `docs/project-decisions.md` D-028 Supersession Note; D-032. Closes the deferred uniqueness question left open by the A2 read-only audit (2026-07-24, main d992c92: 9 rows, zero duplicates under the canonical tuple, zero casing/null issues).

**Decision:** For native pHEX Phase 1 (D-032), the canonical identity of a `RawEndedHexStakeObservation` is exactly `(chainId, lowercase walletAddress, stakeId)`. Exactly one canonical row per ended stake per wallet per chain is permitted, enforced at the database level by a unique index and at the application level by an identity-only lookup in `persistEndedHexStakeObservation`.

`endBlockNumber`, `endTxHash`, `discoveryMethod`, `stakeIndex`, recovery provenance columns (`evidenceRecovery*`), and completion fields (`lockedDay`, `stakeShares`, `isComplete`, `warnings`) are **evidence and attributes recorded on the canonical row — they are not part of identity**.

Persistence outcomes are now:

- **created** — no canonical row existed; a new row was written.
- **updated** — a canonical row existed but was incomplete; the incoming complete observation upgraded the row in place. The row's identity was not changed.
- **conflict** — a canonical row exists, and the incoming end evidence (`endBlockNumber` and/or `endTxHash`) disagrees with the persisted canonical row. Nothing is created, nothing is overwritten. The persistence layer returns a typed conflict result with an operator-safe reason; the discovery layer counts conflicts as `conflicts` (never as `persisted` or `skipped`) and surfaces a `hexmining-ended-stake-end-evidence-conflict` warning; the operator-triggered discover route exposes the count as an additive `conflicts` field.
- **unchanged** — a canonical row exists with matching or non-conflicting evidence; the row is left as-is (already complete, or the incoming observation is not complete). A complete row is never downgraded.

Concurrent writers that lose the create race receive Prisma error `P2002`; the persistence layer catches only that error code, re-reads by canonical identity, and reconciles by the same identity/evidence rules. All other Prisma and non-Prisma errors propagate unchanged.

**Rationale:** Under Phase 1 (D-032) there is only one native pHEX contract per chain, so the wallet+stakeId tuple is globally unique within `(chainId, walletAddress)`. Prior to this decision the application dedupe key was `(chainId, walletAddress.lower, stakeId, endBlockNumber, discoveryMethod)` and there was no database uniqueness at all — a second observation from a different `discoveryMethod` (e.g., `rpc_history` after `raw_stake_action`) or a re-observation at a different `endBlockNumber` would silently create a second canonical row. That widened dedupe key was documented as an out-of-scope debt in the D-028 Supersession Note and in the recovery runbook.

Making the database enforce `(chainId, walletAddress, stakeId)` uniqueness at the same time the application narrows its lookup to the same tuple:

- Prevents duplicate canonical rows even under concurrent writers (P2002 as the last line of defence).
- Turns end-evidence disagreement into a first-class outcome the operator can see, instead of a silent second row.
- Aligns the application identity with the database constraint so both stay in sync.
- Preserves the merged evidence-completion/recovery lifecycle (PRs #334, #335, #337): an already-persisted canonical row can still be upgraded in place from START-time evidence or historical contract-state recovery — those paths were already identity-bound (`enrichEndedHexStakeObservation` requires the full identity plus `isComplete: false`) and remain unaffected.

**Implications:**

- The DB migration is additive at the row level: it drops the redundant non-unique index on `(chainId, walletAddress, stakeId)` and creates a unique index over the same tuple, with a defensive `DO $$ ... $$` block that aborts closed if any duplicates exist at migration time. The read-only pre-migration audit proved no duplicates existed; no cleanup UPDATE or DELETE ran, and no other column, index, or FK was touched.
- `buildEndedStakeDedupeKey` now takes only `(chainId, walletAddress, stakeId)` and produces `"<chainId>:<lower(walletAddress)>:<stakeId>"`. This matches the database constraint exactly.
- `DiscoverEndedHexStakesResult` gains an additive `conflicts: number` field. The discovery route response envelope gains a corresponding additive `conflicts` field alongside `discovered`, `persisted`, `skipped`, and `warnings`. `schemaVersion` remains `"v1"` (additive change).
- Historical-state recovery (`enrichEndedHexStakeObservation`) and its runner behavior are unchanged; the recovery execute path was **not** run as part of this PR.
- Transfer-backfill Window 61 was **not** run as part of this PR.

**Do not:**

- Do not add `endBlockNumber`, `endTxHash`, `discoveryMethod`, or any recovery/completion field to the canonical identity — they are evidence.
- Do not treat a canonical-identity conflict as a plain skip or as newly persisted — it must remain a distinct outcome counted and surfaced explicitly.
- Do not add HSI, HTT, eHEX, or any other stake contract's rows to this table or this identity model. If those source families are ever added, a separately approved decision may need to extend identity with an explicit contract/source dimension (e.g., a source-contract column) — that expansion is out of scope for D-033.
- Do not weaken the `P2002` guard to swallow other Prisma error codes; only `P2002` reconciles via re-read.
- Do not remove or repurpose the existing recovery provenance columns.
