...
**Status:** Proposed — **not yet Accepted**. This entry records a policy proposal awaiting product-owner review; it is not a settled decision like the other entries in this file. Promote to `Accepted` only after the open questions in `docs/pulsechain-fork-state-policy.md` §21 are resolved (fork-boundary block confirmed via Tier 1 source; stale-docs synchronization completed).

**Evidence:** [E1] `docs/pulsechain-fork-state-policy.md` (this decision's companion document, added in the same PR). [E3] Repository inspection (2026-08-04): no fork-boundary, pre-fork, or inherited-history provenance logic exists anywhere in `src/services/` (ingestion, staking, transfer, normalization, materialization, pricing, PnL); `src/config/assets.ts` defines only PulseChain-side assets (no `eHEX`/`chainId: 1` entry); test fixtures in `tests/services/hexmining/ended-stake-*.test.ts` contain two stake blocks (`15,353,156`, `15,767,882`) below the task-supplied fork-boundary figure (`17,233,000`, unresolved — see below). [E4] Task-provided investigation context (PulseChain RPC chain behavior claims, block-boundary figures) — explicitly not independently re-verified via live RPC in this docs-only session; treated as investigation input, not verified chain fact, until Tier 1-confirmed.

**Decision:** Proposes a five-class provenance model (`PULSECHAIN_POST_FORK`, `ETHEREUM_INHERITED_HISTORY`, `FORK_OPENING_STATE`, `ETHEREUM_CHAIN_HISTORY`, `UNKNOWN_OR_UNVERIFIED`) and selects **Option C — dual-view evidence model**: inherited Ethereum history retained as immutable evidence with explicit provenance; post-fork ledger/materialization/PnL to be built from explicit fork-opening state plus post-fork activity only, once implemented; inherited history never silently blended into ordinary PulseChain transaction history, balances, or future PnL/cost-basis without an explicit fork-opening-state policy. Full rationale, terminology, asset-identity rules (pHEX ≠ eHEX despite shared fork-copied contract address), and nine bounded future implementation phases are recorded in `docs/pulsechain-fork-state-policy.md`.
