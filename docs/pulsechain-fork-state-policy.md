...
**Status:** Proposed — architecture and documentation policy only. Not yet an accepted decision (no `D-0xx` entry exists until `docs/project-decisions.md` is updated in this same PR and reviewed).
...
- Two wallet stake actions exist at block `15,353,156` (`2022-08-16T15:26:09Z`) and block `15,767,882` (`2022-10-17T12:39:59Z`). **Partially re-verified [E3]:** these exact block numbers appear in committed test fixtures — `tests/services/hexmining/ended-stake-observation-store.test.ts`, `tests/services/hexmining/ended-stake-reader.test.ts`, `tests/services/hexmining/ended-stake-historical-state-recovery.test.ts`. Both blocks are below the claimed fork boundary of `17,233,000`, consistent with pre-fork timestamps in 2022 (PulseChain mainnet launched 2023). This session did not independently confirm the same transaction hash exists on both Ethereum and PulseChain — that cross-chain claim is task-provided investigation context, not verified here.
- CoinPulse currently stores these rows under `chainId = 369`. [E3] — `PortfolioStakePosition`, `RawStakeAction`, and all HexMining observation tables are keyed by `chainId`, and the only supported `chainId` in `src/config/chains.ts` and `src/services/hexmining/types.ts` is `369`. There is no separate storage path for pre-fork vs. post-fork evidence.
...
- **`docs/v2-hexmining-roadmap.md` §8 Decision 10 does not exist under that heading.** A search for `Decision 10` in that file returned no matches. The comment in `types.ts:123` cites a section that could not be located as written; this is a documentation drift, not a resolved policy. [E3]
...
Every layer of the truth stack — raw ingestion, canonical ledger, materialized positions, pricing, PnL, HexMining stakes — currently treats `chainId: 369` as a single undifferentiated data space. A row observed at block 15,353,156 (2022, pre-fork, Ethereum-origin) and a row observed at block 30,000,000 (2025, PulseChain-native) are stored, normalized, materialized, and would eventually be priced and valued identically, with no field anywhere recording which side of the fork the underlying event actually occurred on.
...
| Term | Definition |
|---|---|
| **Fork boundary** | The specific PulseChain block (and its corresponding pre-fork Ethereum block) below which chain state was inherited wholesale from Ethereum at PulseChain's genesis, and at or above which PulseChain's own validators independently produced blocks. |
| **Inherited Ethereum history** | Transactions, events, and state that occurred on Ethereum before the fork boundary and are visible today through a PulseChain RPC only because PulseChain's genesis copied Ethereum's full state/history. |
| **Fork-opening state** | The balance and contract-storage snapshot that existed on PulseChain the moment the fork boundary was crossed — the starting point for any *independently PulseChain* accounting, as distinct from the Ethereum history that produced it. |
| **Post-fork PulseChain activity** | Transactions, events, and state produced independently by PulseChain validators at or after the fork boundary. |
...
| **Provenance class** | A categorical label attached to an observation or ledger entry describing which side of the fork boundary — and with what confidence — the underlying event originated from. |

---

## 6. Fork boundary

**Canonical boundary definition (proposed):** the fork boundary is a **PulseChain block number**, not a calendar date. Calendar dates are unreliable for this purpose because block-production rate is not perfectly uniform and because timestamps alone cannot distinguish "an Ethereum block replayed at genesis" from "a PulseChain block produced independently around the same historical date" without also checking block number against the known boundary.

- **Last inherited/pre-fork block:** the highest PulseChain block number that represents copied Ethereum state, not independently PulseChain-produced state. The task's investigation context gives `17,233,000` as the point below which blocks are available through the PulseChain RPC; this document treats that specific number as an **unresolved question requiring Tier 1 operator verification** (§21), not as an accepted constant, because it was not independently re-verified via live RPC access in this docs-only session and no Tier 1 PulseChain source (`docs/pulsechain-authoritative-data-sources.md` §1) was consulted for it in this session.
- **First independent PulseChain block:** the fork boundary block number plus one, by definition, once the boundary itself is confirmed.
- **Inclusive/exclusive convention (proposed):** the boundary block itself should be treated as the **last inherited block** (inclusive of inherited-history classification), and the boundary-plus-one block as the **first post-fork block** (exclusive of inherited-history classification). This mirrors the existing repository convention in `docs/transfer-history-backfill-operator-plan.md` fact 5, where block ranges are always treated as fully inclusive on both ends — the same inclusive-range discipline should extend to the fork-boundary classification rather than introducing a second, differently-scoped convention.
- **Timestamp and block-hash interpretation below the boundary (proposed):** a block's `timestamp` and `blockHash` below the fork boundary describe the **original Ethereum block** that PulseChain's genesis copied — they are not evidence of *when on PulseChain's own timeline* the event became visible (PulseChain did not exist yet). Any future implementation must document this explicitly wherever a pre-fork timestamp is surfaced, so it is never read as "this happened on PulseChain at this time."
- **Do not rely only on calendar dates (explicit constraint honored):** the fork boundary must be implemented as a block-number comparison. A calendar-date heuristic (e.g., "before PulseChain's 2023 mainnet launch") is an acceptable *sanity cross-check* only, never the primary classification mechanism, because block-timestamp data below the boundary describes Ethereum's timeline, not PulseChain's.
...
| Class | Meaning |
|---|---|
| `PULSECHAIN_POST_FORK` | The observation's block number is at or above the first independent PulseChain block (§6). Produced by PulseChain's own validators. This is the only class eligible for ordinary PulseChain transaction-history, ledger, and (once implemented) PnL/cost-basis treatment without qualification. |
| `ETHEREUM_INHERITED_HISTORY` | The observation was returned by a PulseChain RPC, is stored under `chainId: 369`, but its block number is at or below the fork boundary (§6) — i.e., it is Ethereum-origin history that PulseChain's genesis copied and continues to serve. This is the class that today's ingestion silently mislabels as ordinary PulseChain activity. |
| `FORK_OPENING_STATE` | Not a transaction-level observation at all — a distinct, separately-evidenced balance/contract-storage snapshot representing the state that existed at the moment the fork boundary was crossed (§10). This class has no rows in any current CoinPulse table; it does not exist yet. |
...
**Explicit non-action (required by the task):** this policy does not delete, hide, or relabel any row that exists in the database today. No such action occurs in this PR, and any future implementation PR must not silently reclassify existing evidence — it must add a new, additive provenance field, defaulting to `UNKNOWN_OR_UNVERIFIED` for existing rows, backfilled only through an explicit, reviewed, read-only audit pass (§19 Phase 2).
...
**Conceptual source of truth and required evidence:** Fork-opening state is not something CoinPulse can compute from its own ledger, because CoinPulse's ledger only begins accumulating PulseChain-scoped evidence from whenever ingestion started — it is not itself the source of truth for "what existed at the fork boundary." The source of truth must be **the actual PulseChain chain state at the confirmed fork-boundary block**, read via a Tier 1 PulseChain RPC/state-read at that exact historical block (analogous to the existing `stakeLists` at `endBlockNumber - 1` pattern already used in `ended-stake-historical-state-recovery.ts` for a different purpose) — not re-derived from Ethereum mainnet data, and not assumed from any documentation, blog post, or third-party claim about "what PulseChain copied."

**Explicit non-fabrication (required by the task):** this document does not propose, estimate, or record any fork-opening balance for any wallet or asset. No such figure exists anywhere in this PR. A future implementation PR must obtain fork-opening state exclusively through a Tier 1-verified, block-pinned, on-chain state read — never through inference, estimation, or a third-party balance snapshot service.
...
- **Opening-state evidence** (`FORK_OPENING_STATE`) — a new, distinct evidence type representing a snapshot balance/position at the fork boundary, sourced per §10; would need its own raw/canonical representation, separate from ordinary `LedgerEntry` rows which represent discrete transaction-driven events, not point-in-time snapshots.
...
- **Pre-fork Ethereum stake events:** A `RawStakeAction` START record whose block number falls below the confirmed fork boundary represents a stake that was actually opened on Ethereum, not PulseChain. Under this policy, such a START record's provenance is `ETHEREUM_INHERITED_HISTORY`, not `PULSECHAIN_POST_FORK`.
- **Copied active stake state:** A stake opened pre-fork that was still active at the fork boundary, and that PulseChain's genesis therefore continued as a live position, is a direct instance of `FORK_OPENING_STATE` (§10) applied to the HexMining domain — its `lockedDay`/`stakeShares`/principal at the fork boundary are the correct opening evidence, not its original Ethereum `StakeStart` parameters reinterpreted as PulseChain-native.
- **Post-fork stake lifecycle:** A stake started and/or ended entirely at or after the fork boundary requires no special fork-state handling beyond the existing native pHEX pipeline (D-032/D-033) — it is fully `PULSECHAIN_POST_FORK`.
...
- **What additional gate is required before it does?** A fork-boundary check on every proposed backfill window: if `[startBlock, endBlock]` includes any block at or below the confirmed fork boundary, the window must not be submitted for execution until (a) the fork boundary itself is Tier 1-confirmed (§6, §21) and (b) fork-opening-state evidence exists for the wallet/assets the window would affect, or an explicit product-owner decision accepts ingesting pre-fork rows as `ETHEREUM_INHERITED_HISTORY`-tagged evidence without yet computing opening state (i.e., evidence-gathering only, no premature ledger/materialization impact).
...
2. Fork-opening state (`FORK_OPENING_STATE`) is a new, separately-evidenced concept that does not exist in CoinPulse today and must be implemented as its own bounded slice (§19 Phase 3), sourced only from Tier 1-verified, block-pinned on-chain state reads (§10) — never fabricated, estimated, or inferred.
...
Each phase below is a separate, bounded future PR. None is implemented by this PR.

1. **Provenance enums/types and boundary helper.** Add the five-value provenance enum (§7) as a backend-only TypeScript type (and, if needed, a Prisma enum) plus a pure function that classifies a block number against a confirmed fork-boundary constant. No ingestion, ledger, or DTO wiring yet — types and a helper only, with unit tests covering boundary edge cases (exactly-at-boundary, one-below, one-above).
2. **Read-only audit of existing pre-fork rows.** A read-only operator script/query (no writes) that reports how many existing raw/ledger rows across all tables fall at or below the confirmed fork boundary, for which wallets/assets, with no mutation of any kind — output is an evidence report, not a migration.
3. **Fork-opening evidence model.** Schema design (proposed, not applied) plus service-layer design for persisting `FORK_OPENING_STATE` snapshots, sourced from block-pinned on-chain reads per §10. This phase is itself likely large enough to warrant its own sub-decision before implementation, given the schema/migration weight.
4. **Ledger/materialization policy implementation.** Wire the provenance classification into ingestion (tag new rows), and update materialization to treat fork-boundary-crossing wallets with an explicit coverage warning until opening-state evidence exists for them, per §11.
5. **API/DTO provenance.** Add provenance/coverage fields to relevant DTOs (transactions, portfolio balances, HexMining stakes) per §15's target concepts, as additive fields with their own schema-version bump and route contract tests, per D-008/D-012.
6. **UI warnings and inherited-history presentation.** Add the separate "inherited history" surface and coverage warnings to relevant frontend screens, consuming only the new DTO fields — no frontend computation, per D-004.
7. **PnL/cost-basis eligibility.** Wire the new provenance/coverage status into the PnL engine's eligibility gating (analogous to `INSUFFICIENT_COST_BASIS`), once PnL itself is otherwise ready to be implemented per the guardrails doc's sequence — this phase is downstream of, and blocked by, that broader PnL readiness work, not a shortcut around it.
8. **Controlled data migration or rematerialization plan.** Once phases 1–4 exist, a bounded, reviewed plan (its own document) for backfilling provenance classification onto already-ingested rows and rematerializing affected wallets — this is explicitly the point where "repair negative balances" work would eventually connect, but that connection is out of scope for this document.
9. **Backfill boundary gates.** Wire the fork-boundary gate described in §14 into the TRANSFERS backfill tooling/runbook, so that a window crossing the fork boundary is mechanically blocked (not just documented as a manual check) until fork-opening-state readiness is confirmed for the affected wallet.

---

## 20. Migration and safety risks

- **Schema risk:** Phases 3 and 5 above involve additive schema changes (new tables/columns for provenance and fork-opening state). Additive-only per repository convention (`AGENTS.md` "Keep schema migrations minimal and additive unless explicitly required") — no destructive migration is proposed anywhere in this document.
...
6. **What is the exact relationship, if any, between this policy's `FORK_OPENING_STATE` evidence type and the existing deferred "fork-copy cost-basis policy" comment in `src/services/hexmining/types.ts:123`** (which cites a `docs/v2-hexmining-roadmap.md` §8 "Decision 10" that could not be located in the current file)? This drift should be reconciled — either by locating/renumbering the referenced section or by updating the comment to reference this document — in a future docs-only follow-up, not silently assumed to already be resolved.
...
- [x] Nine bounded future implementation phases listed, none implemented here.
...
- [ ] Phases 1–9 (§19) implemented as separate bounded PRs, each independently tested and reviewed.
