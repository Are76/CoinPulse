...
**Product direction and Atlas posture (D-035):** CoinPulse is a portfolio tracker first — a fast live view of currently known or covered wallet assets and positions — with historical transactions, PnL, yield, and DeFi analytics as progressive enrichment, not a precondition for the live view. "Currently known or covered" is the accurate description, not "complete": the live DTO (`LiveHoldingsSnapshotDto`, `src/services/portfolio/live-snapshot-types.ts`) declares `coverage: "known_assets_only"` — it only reads tokens already registered in the `Token` table, so unregistered/unknown tokens never appear, and a token whose balance read fails is dropped from `assets` and surfaces only as a `balance-read-failed:<assetId>` warning string, not a value-level unavailable marker. Do not describe the live snapshot as a complete wallet inventory. Atlas (Figma Make) is the intended frontend design system, adopted into the existing app through small bounded PRs per the phased roadmap in `docs/atlas-design-system-integration-plan.md`; it extends, and does not replace, the already-accepted `docs/design/atlas-design-system-v1.md`. This is a planning/documentation decision only — no component, token, CSS, or DTO change has landed from it. Do not describe a Live Portfolio surface as PnL, accounting, or historical truth. [E1] [E2] [E3]
...
**PulseChain fork-state policy posture (2026-08-04, updated same day):** `docs/pulsechain-fork-state-policy.md` and D-036 (`docs/project-decisions.md`) propose — but do not yet accept — a policy for classifying inherited pre-fork Ethereum history (`blockNumber <= lastInheritedBlock`), fork-opening state (a verified post-transition state read, represented as a canonical opening-balance `LedgerEntry` — not a raw read at `lastInheritedBlock`), and post-fork PulseChain activity (`blockNumber >= firstPostForkBlock`), and for keeping pHEX (chain 369) and any future eHEX (chain 1) identity distinct. **The fork-boundary block number is now Tier 1-verified:** `lastInheritedBlock = 17,232,999`, `firstPostForkBlock = 17,233,000`, confirmed via direct `rpc.pulsechain.com` and `api.scan.pulsechain.com` block-header evidence and exported as `PULSECHAIN_FORK_BOUNDARY` in `src/config/chains.ts`, with a pure classifier (`src/services/chains/fork-provenance.ts`) implementing Phase 1 of `docs/pulsechain-fork-state-policy.md` §19 (see §6.1 for the full evidence record). **D-036 is still `Proposed`, not `Accepted`** — Tier 1 boundary confirmation resolves only Open Question 1 of §21; promotion to `Accepted` still requires resolution of archived Decision 10's pHEX cost-basis choice (`docs/v2-hexmining-roadmap-archive.md:449-450`, still open) and the documentation-synchronization follow-up above. No schema, DTO, sync, rebuild, or backfill change has been made; the only code change is the bounded Phase 1 constant/classifier described above. Do not treat any fork-boundary block number other than the Tier 1-verified pair above as confirmed, and do not treat this verification as promoting D-036 to `Accepted`. [E1] [E2]
...
3. Phase 5 ended stake pipeline is complete (persistence, discovery, reader, DTO, API route). Ended stake observations always have `isComplete: false` and `lockedDay: null` **at discovery time** — that discovery-time limitation still holds. A separate, later evidence-based recovery lifecycle is now implemented (#334 start-time evidence persistence, #335 completion from persisted start evidence, #337 historical contract-state recovery), so a discovered row can subsequently become complete; see the recovery entry above and the D-028 Supersession Note. Operator execute/API-verification evidence is now recorded (execute-mode recovery 9/9 with 0 failures; API verification `PASS`) — see D-033 and the evidence templates.
...
CoinPulse product direction and Atlas adoption are defined by D-035 (docs/project-decisions.md) and docs/atlas-design-system-integration-plan.md: portfolio tracker first, fast live view of currently known or covered wallet assets and positions via a fast portfolio path (not a complete wallet inventory — the live DTO's `coverage: "known_assets_only"` excludes unregistered tokens and drops tokens whose balance read failed), historical/PnL/yield/DeFi analytics as progressive enrichment via the existing truth-stack path — not a precondition for the live view. Atlas is the intended frontend design system, adopted via the phased roadmap there; it is docs-only so far, no component/token/CSS/DTO change has landed. Never label a live/estimated value as PnL, accounting, or historical truth.
