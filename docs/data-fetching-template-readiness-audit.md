...
The strongest readiness evidence is the implemented TanStack Query foundation (`src/lib/query/query-client.ts`, `src/components/providers/query-provider.tsx`), shared query keys (`src/lib/query/query-keys.ts`), DTO-first read hooks (`src/lib/query/use-dashboard-query.ts`, `src/lib/query/use-debug-health-query.ts`, `src/lib/query/use-debug-status-query.ts`, `src/lib/query/use-tracked-wallets-query.ts`, `src/lib/query/use-pricing-status-query.ts`), operator mutation hooks (`src/lib/query/use-manual-sync-mutation.ts`, `src/lib/query/use-rebuild-mutation.ts`, `src/lib/query/use-wallet-import-mutation.ts`), and scoped invalidation helper (`src/lib/query/invalidation.ts`). The strongest blockers are stability-window proof, production-like full-cycle evidence for operator flows, the six remaining route-level contract-test gaps recorded in `docs/route-contract-coverage-index.md`, and the absence of a generalized pattern reference that strips CoinPulse-specific domain details before any template is created. PR #106 closed the current G7 hook/mutation coverage gaps; that closure does not authorize internal template folders, external repositories, reusable code extraction, or future-hook coverage claims.
...
| G6. Route contract tests exist for success and failure paths. | mostly met for complete V1 route coverage; met for coverage visibility | Success and failure/error-envelope evidence exists for focus DTO routes: `tests/api/portfolio-dashboard-route-contract.test.ts` and `tests/api/portfolio-dashboard-route.test.ts` for `GET /api/portfolio/dashboard`; `tests/api/wallets-tracked-route-contract.test.ts` for `GET /api/wallets/tracked`; `tests/api/debug-status-route-contract.test.ts` for `GET /api/debug/status`; `tests/api/debug-health-route.test.ts` for `GET /api/debug/health`; and `tests/api/prices-status-route-contract.test.ts` for `GET /api/prices/status`. Import and backend operation route contracts are covered by `tests/api/wallet-import-route-contract.test.ts` and `tests/api/backend-operation-routes.test.ts`. `docs/route-contract-coverage-index.md` now gives reviewers a route-by-route coverage index. | Keep this blocker open for complete route coverage until the six missing route-level tests in `docs/route-contract-coverage-index.md` are added for debug health, manual sync, and rebuild validation/not-found/internal-error branches. Every future DTO route must also keep success, invalid input where applicable, not-found where applicable, and safe failure envelopes. |
...
5. Close the six missing route-level tests documented in `docs/route-contract-coverage-index.md`.
6. Draft a CoinPulse-specific-vs-reusable rule separation note for `AGENTS.md` before copying workflow rules anywhere.
...
- G6 blocker: six missing route-level tests remain for debug health, manual sync, and rebuild validation/not-found/internal-error branches as recorded in `docs/route-contract-coverage-index.md`.
...
1. Add the six missing G6 route-level tests documented in `docs/route-contract-coverage-index.md`.
