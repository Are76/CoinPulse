...
- Branch: `docs/data-fetching-template-readiness-audit`.
...
The strongest readiness evidence is the implemented TanStack Query foundation (`src/lib/query/query-client.ts`, `src/components/providers/query-provider.tsx`), shared query keys (`src/lib/query/query-keys.ts`), DTO-first read hooks (`src/lib/query/use-dashboard-query.ts`, `src/lib/query/use-debug-health-query.ts`, `src/lib/query/use-debug-status-query.ts`, `src/lib/query/use-tracked-wallets-query.ts`, `src/lib/query/use-pricing-status-query.ts`), operator mutation hooks (`src/lib/query/use-manual-sync-mutation.ts`, `src/lib/query/use-rebuild-mutation.ts`, `src/lib/query/use-wallet-import-mutation.ts`), scoped invalidation helper (`src/lib/query/invalidation.ts`), current G6 route-level contract coverage, and current G7 hook/mutation coverage. The strongest blockers are stability-window proof, production-like full-cycle evidence for operator flows, and the absence of a generalized pattern reference that strips CoinPulse-specific domain details before any template is created. PR #106 closed the current G7 hook/mutation coverage gaps, and PR #108 closed the six current G6 route-level gaps recorded in `docs/route-contract-coverage-index.md`; those closures do not authorize internal template folders, external repositories, reusable code extraction, or future-route/future-hook coverage claims.
...
| G6. Route contract tests exist for success and failure paths. | complete for current V1 routes | Success and failure/error-envelope evidence exists for current DTO/support routes: `tests/api/portfolio-dashboard-route-contract.test.ts` and `tests/api/portfolio-dashboard-route.test.ts` for `GET /api/portfolio/dashboard`; `tests/api/wallets-tracked-route-contract.test.ts` for `GET /api/wallets/tracked`; `tests/api/debug-status-route-contract.test.ts` for `GET /api/debug/status`; `tests/api/debug-health-route.test.ts` for `GET /api/debug/health`; and `tests/api/prices-status-route-contract.test.ts` for `GET /api/prices/status`. Import and backend operation route contracts are covered by `tests/api/wallet-import-route-contract.test.ts` and `tests/api/backend-operation-routes.test.ts`. `docs/route-contract-coverage-index.md` gives reviewers a route-by-route coverage index and records no open gaps for the current implemented V1 route set after PR #108. | No current G6 blocker remains for implemented V1 routes. Every future DTO/API route must be added to `docs/route-contract-coverage-index.md` with success coverage plus invalid input where applicable, not-found where applicable, conflict where applicable, and safe failure envelopes. This status does not claim internal template folder readiness, external repo readiness, or reusable code extraction readiness. |
| G7. Query hooks and mutation hooks have consistent test coverage. | complete for current hooks | Query client defaults are tested in `tests/lib/query-client.test.ts`; query key normalization is tested in `tests/lib/query-keys.test.ts`. Read hooks have tests for query key usage, stale/gc timing, fetch invocation, disabled behavior, and backend error preservation in `tests/lib/use-dashboard-query.test.ts`, `tests/lib/use-debug-health-query.test.ts`, `tests/lib/use-debug-status-query.test.ts`, `tests/lib/use-tracked-wallets-query.test.ts`, and `tests/lib/use-pricing-status-query.test.ts`. PR #106 closed the current dashboard-hook gap by adding exact `queryKeys.dashboard(...)`, `DASHBOARD_STALE_TIME`, and `DASHBOARD_GC_TIME` assertions. Mutation hooks have tests for client invocation, no retry by behavior, error preservation, scoped invalidation, exact invalidation call counts, unrelated-family exclusions, and non-blocking invalidation in `tests/lib/use-manual-sync-mutation.test.ts`, `tests/lib/use-rebuild-mutation.test.ts`, `tests/lib/use-wallet-import-mutation.test.ts`, and `tests/lib/invalidation.test.ts`. `docs/hook-mutation-test-checklist.md` records the read-hook, mutation-hook, invalidation-helper, and query-key checklist. | No current G7 hook/mutation blocker remains. Future hooks must add their own checklist coverage when introduced, and future filtered keys/object semantics remain future-only until production-used. This status does not claim internal template folder readiness, external repo readiness, or reusable code extraction readiness. |
...
5. Draft a CoinPulse-specific-vs-reusable rule separation note for `AGENTS.md` before copying workflow rules anywhere.

G6 status note: PR #108 closed the six current route-level gaps previously recorded in `docs/route-contract-coverage-index.md`. G6 is complete for current V1 routes only; future DTO/API routes must add index entries and success/error coverage when introduced.

G7 status note: PR #106 closed the current hook/mutation coverage gaps and `docs/hook-mutation-test-checklist.md` now records the checklist. G7 is complete for current hooks only; future hooks and future production-used filtered-key object semantics still need their own evidence when introduced.
...
- G6 status: complete for current V1 routes after PR #108; no current route-level blocker remains, but future DTO/API routes must be added to `docs/route-contract-coverage-index.md` with success and applicable error coverage.
- G7 status: complete for current hooks after PR #106; no current hook/mutation blocker remains, but future hooks and future production-used filtered-key object semantics must add their own checklist coverage when introduced.
...
1. Record the G1 release-cycle stability checkpoint for dashboard wallet selection and submitted dashboard query behavior.
2. Add a documentation-only dashboard DTO stability checkpoint after the next qualifying dashboard-route PRs.
3. Add a documentation-only tracked-wallet stability checkpoint after more bounded PRs exercise the route, hook, and import invalidation policy without shape changes.
4. Run and record a production-like operator sync -> materialize -> rebuild checkpoint, without changing code in the checkpoint PR.
5. Draft the G8 CoinPulse-specific-vs-reusable rule separation note for `AGENTS.md` before copying workflow rules anywhere.
